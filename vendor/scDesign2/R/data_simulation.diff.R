...
#'                        multinomial distribution or follows the exact same proportion as
#'                        specified in \code{cell_type_prop}.
#' @param fold_change     An array with same length as the rows of the count matrix, with fold 
#'                        change values representing how the means should be modified. 
#' @return A matrix of shape p by n that contains the simulated count values. p is derived from
#' \code{model_params}.
#'
#' @export
simulate_count_scDesign2 <- function(model_params, n_cell_new, cell_type_prop = 1,
                                     ...
                                     cell_sample = FALSE, fold_change = NULL){
  sim_method <- match.arg(sim_method)
  ...
  if(!is.null(fold_change)){
    for(iter in 1:n_cell_type)
      if(n_cell_each[iter] > 0){
        params_new <- model_params[[iter]]
        params_new$marginal_param1[, 3] <- params_new$marginal_param1[, 3] * fold_change[params_new$gene_sel1]
        if(sim_method == 'copula'){
          params_new$marginal_param2[, 3] <- params_new$marginal_param2[, 3] * fold_change[params_new$gene_sel2]
        }
        model_params[[iter]] <- params_new
      }
  }
